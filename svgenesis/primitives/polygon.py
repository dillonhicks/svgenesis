...
class Polygon(AbstractPoly):
    """
    Defines a polygon by its vertex points.
    """

    SVG_TAG_NAME = 'polygon'

    @staticmethod
    def xml_parse_handler(xml_elem):
        return AbstractPoly.xml_parse_handler(xml_elem)

    def __init__(self, points=[], stroke_width=3, stroke_opacity=1.0, 
                 opacity=1.0, stroke=rgb(0,0,0), fill=rgb(0,0,0)):
        """
